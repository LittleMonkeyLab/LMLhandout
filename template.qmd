---
title: "Deception Handout"
subtitle: "StuntLecture"
affiliation: "LittleMonkeyLab"
format: LMLhandout-pdf
---

This is an example of a document in the `LMLhandout` format. It is designed for handouts that students will write on, therefore it maximizes writing space with a small title block, smaller font, and slimmer margins.

How it works:

1. It uses the `fancyhdr` package to create a very slim horizontal title block at the top of the document followed by a horizontal rule.
2. The title block recognizes three yaml keys: `title`, `subtitle` and `affiliation`. They will display in the title block as `affiliation subtitle: title`.
